...
> 更新时间: 2026-07-19 | 状态: 生效 | 文档规范另见 [docs/README.md](README.md)
...
- **工具链**: uv 管理依赖（lock 文件提交）；`ruff check` + `ruff format` 双绿（配置进 pyproject.toml）；`pytest` 必须可离线跑（外部依赖 mock）。
...
- 写操作（POST/PUT/DELETE）保持限流覆盖；Web 表单与 API 同源同限（SR-05 教训）。
