...
选 Redis Streams：两条流 `shareo:stream:index_post`、`shareo:stream:bot_tasks`，消费组 `ai-workers`；消费成功才 XACK，worker 启动时 XAUTOCLAIM 接管 pending 消息，全部消费逻辑幂等（at-least-once）。
...
- 无死信队列 → 约定重试 3 次仍失败的消息记录日志并 XACK 跳过，靠回填命令兜底。
