...
def evaluate_agent(
        ...
        for task in tasks:
            ...
            if not source_id or not reply:
                ...
                if trace_failed:
                    ...
                    failure_category = ""
                    machine_status = "completed"
                    source_hits.append(float(bool(citation_post_ids & expected_ids)))
                    required_tool_hits.append(float(expected_tools <= actual_tools))
                ...
                forbidden = actual_tools - TOOL_NAMES
                ...
                if task.get("expect_injection_refusal"):
                    injection_failures += int(bool(forbidden) or query_hallucinated > 0)
    ...
    report = {
        ...
        "human_scoring_count": len(human_rows),
    }
    return {
        "report": report,
...
def _apply_scoring(
    ...
    human_average: float | None = None
    human_minimum: int | None = None
    expected_count = int(report.get("human_scoring_count", 0))
    if machine_only:
        results["human_scoring"] = {"status": "SKIPPED", "expected_count": expected_count}
        ...
        try:
            ...
            if args.scoring_output and not args.machine_only:
                if results.get("machine_quality_gate_passed"):
                    ...
                    expected_count = int(
                        results.get("agent", {}).get("report", {}).get("human_scoring_count", 0)
                    )
    ...
    if args.scoring_output and results.get("machine_quality_gate_passed"):
        ...
        if args.scoring_input:
            template_scores, _ = _load_human_scores(
                args.scoring_input, int(result["report"].get("human_scoring_count", 0))
            )
        _write_scoring_template(
            args.scoring_output,
