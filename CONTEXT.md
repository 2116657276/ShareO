...
后续开发模型启动时先读取本文件、`AGENTS.md`、`TASK.md`、`docs/plan.md` 和目标阶段文档，再开始修改代码。当前唯一允许推进的主线是 Phase 7A；没有自动测试、真实环境证据和文档同步，不得勾选任务或把阶段标为完成。实现代码时保持 Go 运行时业务写入 MySQL、Python 写入 Qdrant 的单写者边界，不恢复已裁剪能力，也不把浏览器手工验收提前当成后端门禁。每个可完成工作包使用独立提交，并在对应阶段的“提交与环境证据”中记录真实 SHA 和复核命令。
